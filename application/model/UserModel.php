...
class UserModel
{
     ...
     */
    public static function getSelectedCategorys($user_id){
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "Select user_selectedCategorys FROM users WHERE user_id = :user_id";
        $sth = $database->prepare($sql);
        $sth->execute(array(':user_id' => $user_id));

        $selectedCategorys = "";
        while($result = $sth->fetch()){
            $selectedCategorys = $result->user_selectedCategorys;
        }

        return $selectedCategorys;
    }

    public static function categoriesToSession($user_selectedCategorys){
        if (strpos($user_selectedCategorys, 'politics') !== false){
            Session::set('cat_politics', true);
        } else{
            Session::set('cat_politics', false);

        }
        if (strpos($user_selectedCategorys, 'entertainment') !== false){
            Session::set('cat_entertainment', true);
        } else{
            Session::set('cat_entertainment', false);

        }
        if (strpos($user_selectedCategorys, 'sports') !== false){
            Session::set('cat_sports', true);
        } else{
            Session::set('cat_sports', false);

        }
        if (strpos($user_selectedCategorys, 'technology') !== false){
            Session::set('cat_technology', true);
        } else{
            Session::set('cat_technology', false);

        }
        if (strpos($user_selectedCategorys, 'business') !== false){
            Session::set('cat_business', true);
        } else{
            Session::set('cat_business', false);

        }
        if (strpos($user_selectedCategorys, 'learn') !== false){
            Session::set('get_learn', true);
        } else{
            Session::set('get_learn', false);

        }
    }

    public static function setSelectedCategorys($selectedCategory, $user_id){
        var_dump($selectedCategory);
        $database = DatabaseFactory::getFactory()->getConnection();
        $sql = "UPDATE users SET user_selectedCategorys = :selected_Category WHERE user_id = :user_id;";
        $sth = $database->prepare($sql);
        $sth->execute(array(":user_id" => $user_id, ":selected_Category" => $selectedCategory));
    }
    

    public static function getAllCategoryPercentages($user_id){
        $database = DatabaseFactory::getFactory()->getConnection();


            $sql = "Select user_clickedPolitics, user_clickedEntertainment, user_clickedSports, user_clickedTechnology, user_clickedBusiness FROM users WHERE user_id = :user_id";
            $sth = $database->prepare($sql);
            $sth->execute(array( ':user_id' => $user_id));
            while($result = $sth->fetch()){
                $PolPerc = $result->user_clickedPolitics;
                $EntPerc= $result->user_clickedEntertainment;
                $SpoPerc= $result->user_clickedSports;
                $TechPerc= $result->user_clickedTechnology;
                $BusPerc= $result->user_clickedBusiness;


                $sum = $PolPerc+$EntPerc+$SpoPerc+$TechPerc+$BusPerc;

                $factor = $sum / 100;
                $PolPerc = $PolPerc / $factor;
                $EntPerc = $EntPerc / $factor;
                $SpoPerc = $SpoPerc / $factor;
                $TechPerc = $TechPerc / $factor;
                $BusPerc = $BusPerc / $factor;

                $returnVal[0] = "user_clickedPolitics=".ceil($PolPerc);
                $returnVal[1]= "user_clickedEntertainment=".ceil($EntPerc);
                $returnVal[2]= "user_clickedSports=".ceil($SpoPerc);
                $returnVal[3]= "user_clickedTechnology=".ceil($TechPerc);
                $returnVal[4]= "user_clickedBusiness=".ceil($BusPerc);
